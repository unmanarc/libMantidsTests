...
bool RPCServerImpl::callbackOnRPCConnect(void *, Socket_StreamBase *sock, const char * remoteAddr, bool secure)
{
    string sComponent = sock->readStringEx<uint8_t>();
    Socket_TLS * tlsSock = (Socket_TLS *)sock;
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    LOG_APP->log0(__func__,Logs::LEVEL_INFO, "Incomming %sRPC connection from (IP=%s,CN=%s,COMPONENT=%s)", secure?"secure ":"", remoteAddr, tlsSock->getTLSPeerCN().c_str(), sComponent.c_str());

    NetStreams::CryptoChallenge cstreams(sock);
...
{
    Socket_TLS * sockRPCListen = new Socket_TLS;
    ...
    Acceptors::MultiThreaded * multiThreadedAcceptor = new Acceptors::MultiThreaded;
...
bool RPCServerImpl::_callbackOnInitFailed(void * obj, Socket_StreamBase * s, const char * cUserIP, bool isSecure)
{
    LOG_APP->log1(__func__,cUserIP,Logs::LEVEL_ERR,  "Incomming RPC Client TLS initialization Failed...");

    for (const auto & i :((Socket_TLS *)s)->getTLSErrorsAndClear())
    {
...
void RPCServerImpl::_callbackOnTimeOut(void * obj, Socket_StreamBase *s, const char * cUserIP, bool isSecure)
{
    LOG_APP->log1(__func__,cUserIP,Logs::LEVEL_ERR,  "Client CN=%s timed out. Not enough threads...", ((Socket_TLS *)s)->getTLSPeerCN().c_str());
    return;
}
